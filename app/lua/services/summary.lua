...
function _M.refresh()
    local period = tonumber( config.configs.summary.request_enable )
...
function _M.pre_run_matcher()
    if config.configs.summary.request_enable ~= true or config.configs["summary_collect_enable"] ~= true then
        return
    end
    
    for i, rule in ipairs( config.configs.summary.rules ) do
        local enable = rule['enable']
        local matcher_config = config.configs['matcher'][ rule['matcher'] ] 
...
function _M.log()
    ...
    if config.configs.summary.request_enable ~= true then
        return
    end
    
    local with_host_info = config.configs.summary.with_host
    ...
    if config.configs.summary.group_temporary_enable == true then
...
function _M.summary_collect()
    -- GENERAL
    ngx.shared.summary_long:incr("total_traffic", 1, 0)
    ngx.shared.summary_short:incr("total_traffic", 1, 0)

    return
end

function _M.summary_long_get()
    local period = config.configs.summary.request_enable
    
    return ngx.shared.summary_long:get_keys(100)
end

function _M.summary_short_get()
    return ngx.shared.summary_short:get_keys(100)
end

function _M.summary_request()
    if config.configs.summary.request_enable ~= true then
        return
    end
...
function _M.collect_long()
    if config.configs.summary.group_persistent_enable == true then
        -- ... existing code ...
    end
end

function _M.collect_short()
    if config.configs.summary.group_temporary_enable == true then
        -- ... existing code ...
    end
end
