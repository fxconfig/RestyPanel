...
config.get_config = function(){
    $.get("./config",function(data,status){
        // Initialize keepalive_rule if not exists
        if (!data.keepalive_rule) {
            data.keepalive_rule = [];
        }
        if (data.keepalive_enable === undefined) {
            data.keepalive_enable = false;
        }
...
config.save_config = function(){
    ...
    var config_data = config.config_vm.$data['config_now'];
    
    // Add address field to keepalive rules
    if (config_data.keepalive_rule) {
        config_data.keepalive_rule.forEach(function(rule) {
            if (rule.node) {
                var parts = rule.node.split(':');
                if (parts.length === 2) {
                    var upstream_name = parts[0];
                    var node_name = parts[1];
                    var upstream = config_data.backend_upstream[upstream_name];
                    if (upstream && upstream.node && upstream.node[node_name]) {
                        var node = upstream.node[node_name];
                        rule.address = node.host + ':' + (node.port || '80');
                    }
                }
            }
        });
    ...
    var config_json = JSON.stringify(config_data, null, 2);
